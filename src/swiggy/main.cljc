(ns swiggy.main
  "Kotodama WASM entrypoint for the Swiggy clean-room actor (L4) — Clojure port.
  ...
  facts (`swiggy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "swiggy")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"      :plural "orders"      :id-prefix "swiggy_ord"
   ...
   {:entity "Tracking"   :plural "trackings"   :id-prefix "swiggy_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"swiggy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "swiggy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :swiggy-compat/ready)
